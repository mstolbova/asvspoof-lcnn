...
from src.metrics.eer import compute_eer_numpy
...
class BaseTrainer:
    ...
    def _evaluation_epoch(self, epoch, part, dataloader):
        ...
        This version computes GLOBAL EER over the whole validation/test
        partition instead of averaging batch-wise EER values.
        """
        self.is_train = False
        self.model.eval()
        ...
        with torch.no_grad():
            for batch_idx, batch in tqdm(
                ...
                if "logits" in batch and "labels" in batch:
                    ...
                    all_labels.append(batch["labels"].detach().cpu().long())
        ...
        # Compute proper global EER for the whole partition.
        if len(all_logits) > 0 and len(all_labels) > 0:
            ...
            if len(bonafide_scores) > 0 and len(spoof_scores) > 0:
                eer, threshold = compute_eer_numpy(bonafide_scores, spoof_scores)
                logs["eer"] = float(eer)
                logs["eer_threshold"] = float(threshold)

                if self.writer is not None:
                    self.writer.add_scalar("eer_global", float(eer))
                    self.writer.add_scalar("eer_threshold", float(threshold))
            else:
                self.logger.warning(
                    f"Cannot compute global EER for '{part}': "
                    "only one class is present in this partition."
                )
        else:
            self.logger.warning(
                f"Cannot compute global EER for '{part}': "
                "logits or labels were not collected."
            )
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        checkpoint_lr_scheduler_config = checkpoint["config"].get("lr_scheduler")
        current_lr_scheduler_config = self.config.get("lr_scheduler")

        # load optimizer state from checkpoint only when optimizer type is not changed.
        if (
            checkpoint["config"]["optimizer"] != self.config["optimizer"]
            or checkpoint_lr_scheduler_config != current_lr_scheduler_config
        ):
            self.logger.warning(
